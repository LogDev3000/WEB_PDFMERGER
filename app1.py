...
def main():
    ...
    st.header('UNIR PDFs')
    st.subheader('Adjunta tus archivos PDF')


    attached_pdfs = st.file_uploader(label = '',accept_multiple_files=True )

    merge_button = st.button(label='MEZCLAR PDFs')

    if merge_button:

        if len(attached_pdfs ) <= 1:

            st.warning('Adjunta más de un PDF')
            ...
            with open(output_pdf, 'rb') as file:
                pdf_data = file.read()
            st.download_button(label = 'DESCARGAR PDF MEZCLADO', data=pdf_data, file_name='pdf_final.pdf')
